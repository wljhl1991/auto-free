...
const DEFAULT_ENDPOINT: &str = "https://music.163.com/api/music/generate";
const DEFAULT_MODEL: &str = "tianyin-v1";
...
pub struct NeteaseMusicProvider {
...
impl NeteaseMusicProvider {
    pub fn new(config: &AIProviderConfig, asset_base_path: PathBuf) -> Result<Self, ProviderError> {
        let api_key = config.auth_config.api_key
            ...
            .map(|k| k.value.clone())
            .filter(|v| !v.is_empty())
            .ok_or_else(|| ProviderError::InvalidConfig("网易天音 API Key 未配置".to_string()))?;
    ...
    pub async fn generate_music(
        ...
        log::info!("[NeteaseMusic] 请求: endpoint={}, model={}, prompt_len={}, duration={:?}, style={:?}", 
        ...
        let response = self.client
            ...
            .await
            .map_err(|e| {
                log::error!("[NeteaseMusic] 请求发送失败: {} (is_timeout={}, is_connect={}, is_request={})", 
        ...
        log::info!("[NeteaseMusic] 响应状态: {} {}", status.as_u16(), status.canonical_reason().unwrap_or(""));

        let body = response.text().await
            .map_err(|e| {
                log::error!("[NeteaseMusic] 读取响应体失败: {}", e);
        ...
        log::info!("[NeteaseMusic] 响应体: {}", truncated_response);
        save_raw_response("netease_music", "music_gen", &body);
        ...
        let gen_response: MusicGenerationResponse = serde_json::from_str(&body)
            .map_err(|e| {
                log::error!("[NeteaseMusic] 解析响应JSON失败: {} (原始响应前200字: {})", e, truncate_str(&body, 200));
                ProviderError::GenerationFailed(format!("解析响应失败: {}", e))
            })?;

        let task_id = gen_response.task_id;
        log::info!("[NeteaseMusic] 任务已提交: task_id={}", task_id);
    ...
    async fn poll_until_complete(&self, task_id: &str) -> Result<String, ProviderError> {
        ...
        loop {
            ...
            if elapsed >= MAX_POLL_DURATION_SECS {
                log::error!("[NeteaseMusic] 轮询超时: task_id={}, elapsed={}s", task_id, elapsed);
            ...
            log::info!("[NeteaseMusic] 轮询状态: task_id={}, elapsed={}s", task_id, elapsed);
            ...
            let response = self.client
                ...
                .await
                .map_err(|e| {
                    log::error!("[NeteaseMusic] 轮询请求失败: {}", e);
            ...
            let body = response.text().await
                .map_err(|e| {
                    log::error!("[NeteaseMusic] 读取轮询响应失败: {}", e);
            ...
            log::info!("[NeteaseMusic] 轮询响应: status={}, body={}", status.as_u16(), truncated);
            save_raw_response("netease_music", "music_gen_query", &body);
            ...
            let task_status: MusicTaskStatusResponse = serde_json::from_str(&body)
                .map_err(|e| {
                    log::error!("[NeteaseMusic] 解析轮询响应失败: {}", e);
                    ProviderError::GenerationFailed(format!("解析轮询响应失败: {}", e))
                })?;

            match task_status.status.as_str() {
                "completed" => {
                    log::info!("[NeteaseMusic] 音乐生成完成: task_id={}", task_id);
                    return task_status.url
                        .ok_or_else(|| {
                            log::error!("[NeteaseMusic] 音乐生成完成但未返回音频URL");
                            ProviderError::GenerationFailed("音乐生成完成但未返回音频URL".to_string())
                        });
                }
                "failed" => {
                    log::error!("[NeteaseMusic] 音乐生成失败: task_id={}, error={:?}", task_id, task_status.error);
    ...
    async fn download_audio(&self, url: &str) -> Result<Vec<u8>, ProviderError> {
        log::info!("[NeteaseMusic] 下载音频: url={}", &url[..url.len().min(200)]);
        ...
        let response = self.client
            ...
            .await
            .map_err(|e| {
                log::error!("[NeteaseMusic] 下载音频失败: {}", e);
                ProviderError::NetworkError(format!("下载音频失败: {}", e))
            })?;

        let status = response.status();
        if !status.is_success() {
            log::error!("[NeteaseMusic] 下载音频响应错误: status={}", status);
        ...
        let data = response.bytes().await
            .map(|b| b.to_vec())
            .map_err(|e| {
                log::error!("[NeteaseMusic] 读取音频数据失败: {}", e);
                ProviderError::NetworkError(format!("读取音频数据失败: {}", e))
            })?;

        log::info!("[NeteaseMusic] 音频下载完成: size={}KB", data.len() / 1024);
        Ok(data)
    }
    ...
    fn infer_style(&self, prompt: &str) -> String {
        ...
        if lower.contains("horror") || lower.contains("恐怖") {
            ...
            "romantic".to_string()
        } else if lower.contains("battle") || lower.contains("战斗") {
            "epic orchestral".to_string()
        } else if lower.contains("mystery") || lower.contains("悬疑") {
            "mysterious ambient".to_string()
        } else if lower.contains("happy") || lower.contains("欢快") {
            "upbeat".to_string()
        } else if lower.contains("sad") || lower.contains("悲伤") {
    ...
    fn handle_error_status(&self, status_code: u16, body: &str) -> ProviderError {
        ...
        log::error!("[NeteaseMusic] API错误: status={}, message={}", status_code, error_msg);
...
#[async_trait]
impl IAssetProvider for NeteaseMusicProvider {
