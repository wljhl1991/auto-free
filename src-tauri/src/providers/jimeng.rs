...
const DEFAULT_ENDPOINT: &str = "https://api.jimeng.ai/v1/images/generations";
const DEFAULT_MODEL: &str = "jimeng-2.1";
...
#[async_trait]
impl IAssetProvider for JimengProvider {
    ...
    async fn check_connectivity(&self) -> Result<ConnectivityCheck, ProviderError> {
        ...
        match result {
            Ok(image_url) => {
                // 下载并保存测试图片
                let media_url = self.download_and_save_test_image(&image_url, "jimeng").await.ok();
