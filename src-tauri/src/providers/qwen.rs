...
impl QwenProvider {
    pub fn new(config: &AIProviderConfig, asset_base_path: &std::path::Path) -> Result<Self, ProviderError> {
        let api_key = config.auth_config.api_key
            ...
            .map(|k| k.value.clone())
            .filter(|v| !v.is_empty())
            .ok_or_else(|| ProviderError::InvalidConfig("Qwen API Key 未配置".to_string()))?;
    ...
    async fn send_request(&self, request: &ChatRequest) -> Result<String, ProviderError> {
        log::info!("[Qwen] 请求: endpoint={}, model={}, messages_count={}", 
        ...
        log::debug!("[Qwen] 请求体: {}", truncated_body);
        ...
        let response = self.client
            ...
            .await
            .map_err(|e| {
                log::error!("[Qwen] 请求发送失败: {} (is_timeout={}, is_connect={}, is_request={})", 
        ...
        log::info!("[Qwen] 响应状态: {} {}", status.as_u16(), status.canonical_reason().unwrap_or(""));

        let body_bytes = response.bytes().await
            .map_err(|e| {
                log::error!("[Qwen] 读取响应体失败: {}", e);
    ...
    async fn send_stream_request(&self, request: &ChatRequest) -> Result<reqwest::Response, ProviderError> {
        log::info!("[Qwen] 流式请求: endpoint={}, model={}", self.endpoint, request.model);
    ...
    fn handle_error_status(&self, status_code: u16, body: &str) -> Result<String, ProviderError> {
        ...
        log::error!("[Qwen] API错误: status={}, message={}", status_code, error_msg);
...
#[async_trait]
impl IAssetProvider for QwenProvider {
