...
项目使用两个文件协同管理开发进度，当用户发出"按 todo 列表开发"或类似指令时，SOLO 应自动联动这两个文件。

### 文件职责

| 文件 | 用途 | 格式 |
|------|------|------|
| `TODO.md` | 临时功能需求、bug 反馈、优化建议 | `- [ ]` / `- [x]` checkbox |
| `DEVELOPMENT_PLAN.md` | 27 个规划开发节点，按顺序推进 | 节点总览表的状态列（☐ / 🔲 / ☑） |

### 工作流规则

1. **收到开发指令时**：
   - 先检查 `TODO.md` 中是否有未完成的待办项
   - 再查看 `DEVELOPMENT_PLAN.md` 节点总览表，找到当前应推进的节点（第一个状态为 ☐ 的节点）
   - 优先处理 TODO.md 中的紧急 bug，再按开发计划推进

2. **编码完成后**：
   - 如果完成的是 TODO.md 中的条目 → 将 `- [ ]` 改为 `- [x]`
   - 如果完成的是开发计划中的节点 → 将节点总览表中对应行的状态从 ☐ 改为 ☑
   - 如果 TODO 条目对应某个开发节点 → 两边都标记完成

3. **新增待办时**：
   - 用户反馈 bug 或提出功能需求 → 添加到 `TODO.md` 对应分类下
   - 如果该待办对应某个开发节点，在 TODO 条目后标注 `→ 节点 XX`

4. **状态标记说明**（DEVELOPMENT_PLAN.md 节点总览表）：
   - ☐ 未开始
   - 🔲 进行中
   - ☑ 已完成
